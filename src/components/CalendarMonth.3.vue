<template>
  <div id="CalendarMonth">
      ...
      <table>
        ...
        <tbody>
          <tr v-for="(tr,rowindx) in the42data" :key="rowindx">
            ...
            >
              <span class="tip" v-if="col.schedule.length > 3">...{{col.schedule.length - 3}} more</span>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <!-- 核心数据 -->
    <div class="schedule_box scheduleBox">
...
export default {
  ...
  methods: {
    ...
    setCanlendar: function() {
      ...
      API.calendarAPI.getMonthMsg().then(monthMsg => {
        if (monthMsg) {
